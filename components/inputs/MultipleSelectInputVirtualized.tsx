    ...
    >
      {({ index, style }) => (
        <div
          ...
          className={`flex w-full cursor-pointer items-center rounded p-1 px-2 hover:bg-gray-100 ${selectedIds?.includes(list[index] ? list[index].id : 0) ? "bg-gray-100" : ""}`}
        >
          <p className="grow text-sm font-medium text-[#353432]">{list[index]?.label}</p>
          {selectedIds?.includes(list[index] ? list[index].id : 0) ? <HiCheck style={{ color: "#fead61", fontSize: "20px" }} /> : 0}
        </div>
      )}
    </VariableSizeList>
  );
      ...
      {selectMenuIsOpen ? (
        <div className="scrollbar-thin scrollbar-track-gray-100 scrollbar-thumb-gray-300 absolute top-[75px] z-[100] flex h-[250px] max-h-[250px] w-full flex-col self-center overflow-y-auto overscroll-y-auto rounded-md border border-gray-200 bg-[#fff] p-2 py-1 shadow-sm">
          ...
          <div className="flex w-full flex-col gap-y-1">
            {items ? <List height={180} width={"100%"} list={items} /> : <p className="w-full text-center text-sm italic text-[#353432]">Sem opções disponíveis.</p>}
          </div>
        </div>
      ) : (
        false
